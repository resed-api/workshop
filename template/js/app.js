...
function generateWelcomeMessage() {
    ...
    const innerWidth = boxWidth - 4; // Account for "║  " on both sides
    
    const topBorder = '╔' + '═'.repeat(boxWidth - 2) + '╗';
    const bottomBorder = '╚' + '═'.repeat(boxWidth - 2) + '╝';
    const emptyLine = '║' + ' '.repeat(boxWidth - 2) + '║';
    
    const padLine = (text) => '║  ' + text.padEnd(innerWidth, ' ') + '║';
...
async function loadContent() {
    try {
        ...
        if (response.ok) {
            ...
            console.error('✗ Failed to load sections.json:', response.status);
        }
    } catch (e) {
...
Themes:
  - Click 🎨 Theme button to change color scheme
